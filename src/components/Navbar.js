...
export default function Navbar() {
  ...
  const { connectMetamask, disconnectMetamask } = bindActionCreators(actionCreators, dispatch)
  ...
  return (
    <>
      <header className={`${currentUrl == '/' || currentUrl == '/games/stick-hero/play' ? 'lg:absolute relative' : 'relative'} h-24 w-screen bg-[#0a1f2f] flex items-center`}>
        <div className="xl:container w-full xl:px-20 md:px-12 px-4 mx-auto flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center text-white space-x-8 font-semibold font-body text-lg">
            ...
            {isConnect ? (
              <div onClick={onProfileOpen} className="cursor-pointer hover:border-gray-600 hover:text-white border border-transparent rounded-xl p-2 bg-[#02121d] text-white/80">
          ...
          <ModalBody className="bg-[#0a1f2f] text-white font-body rounded-b">
            <div className="space-y-6 pb-6 pt-10">
              ...
              <div className="flex justify-between items-center">
                <div onClick={() => connectToMetamask()} className="cursor-pointer border-2 border-gray-600 rounded-full hover:border-[#28dbd1]">
      ...
      <Modal isOpen={isProfileOpen} onClose={onProfileClose} size="lg" motionPreset="slideInBottom">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader className="relative bg-[#0a1f2f] text-white font-body rounded-t">
            <div className="flex justify-center">
            ...
            <div onClick={closeProfileModal} className="absolute -right-2 -top-2 bg-[#28dbd1]/50 hover:bg-[#28dbd1]/80 text-black w-8 h-8 rounded-lg flex justify-center items-center cursor-pointer">
              <MdClear size={25} />
            </div>
          </ModalHeader>
          <ModalBody className="bg-[#0a1f2f] text-white font-body rounded-b">
            <div className="border rounded-md p-4 border-gray-700 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={disconnectToMetamask} className="text-blue-400 px-2 py-1 border-[1px] border-blue-400/50 hover:border-blue-400 rounded-lg text-sm">Disconnect</button>
                </div>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <a href={`https://goerli.etherscan.io/address/${account}`} target="_blank" className="flex space-x-2 items-center font-medium text-gray-400 cursor-pointer hover:text-gray-400/70">
                    <FiExternalLink />
